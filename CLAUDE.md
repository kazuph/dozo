...
- `package.json` - dozo CLI本体のバージョン管理

## バージョン管理

**dozo CLIとpluginのバージョンは別管理**:
- dozo CLI: `package.json` の `version`
...
claude plugin marketplace update dozo-plugins

# 2. プラグインを更新
claude plugin update dozo-plugin@dozo-plugins
...
**dozoは人間がブラウザでレビューするための確認ツール**。コードを読ませる場ではなく、**「これで問題ないですよね？」の確認の場**。

### dozoに渡すのはコードではなくREPORT.md
- REPORT.mdに **変更の意図・影響範囲・テスト結果・エビデンス** をまとめる
- それをdozoで開いて、ユーザーが「OK」か「ダメ」か判断できる状態にする
- コードの差分を直接dozoに渡すのは冒涜（ユーザーにコードを読ませるな）

### 正しいフロー
1. 実装 → テスト → エビデンス収集
2. REPORT.mdに「何を・なぜ・どう変えた」「テスト結果」「確認項目」をまとめる
3. `npx dozo .artifacts/<feature>/REPORT.md` でフォアグラウンド起動
4. ユーザーがブラウザで確認 → Approve/Request Changes

### 禁止事項
- `git diff | dozo --diff` でコード差分だけ見せる
- AIエージェント（dozo-plugin:review-code-security等）にレビューさせる
- お膳立てなしでいきなりdozoを開く
...
| 変更規模 | 使うskill | 説明 |
|---|---|---|
| 小さな修正・バグ修正 | `/dozo-plugin:tiny-done` | ビルド → スクショ/テスト結果 → openで開く → ユーザー承認 |
| 大きな機能・複数ファイル変更 | `/dozo-plugin:done` | ビルド → 検証 → REPORT.md作成 → dozoで開く → ユーザー承認 |
...
- 「ご自身で確認されますか？」と聞くのも禁止 → 確認はフォアグラウンドでdozoを開いて行う
- 承認なしでcommit/push禁止
